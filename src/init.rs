...
impl AiService {
    const SERVICES: &'static [AiService] = &[
        ...
        AiService {
            ...
            ultrathink: Some("Use oracle and think heavily"),
        },
        AiService {
            name: "opencode",
    ...
    fn get_config(&self) -> ProjectConfig {
        let ai_apps: Vec<AiApp> = self
            ...
            .map(|(&idx, cmd)| AiApp {
                ...
                command: cmd.clone(),
                ultrathink: AiService::SERVICES[idx]
                    .ultrathink
...
fn save_config(wizard: &WizardState) -> Result<()> {
    ...
    {{
      ...
      "command": "{}"{} 
                ...
                app.name, app.command, ultrathink
            )
        })
        .collect::<Vec<_>>()
        .join(",");
